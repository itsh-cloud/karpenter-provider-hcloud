...
//
// Karpenter core's Requirements.Compatible() ALLOWS a well-known label that a
// NodePool template leaves undefined but DENIES a custom one, and core's
// VolumeTopology injects a bound PV's nodeAffinity keys as NodeClaim
// requirements. So if csi.hetzner.cloud/location is not well-known, every pod
// with an existing hcloud volume becomes permanently unschedulable, with
// nothing in the scheduling error naming Karpenter as the cause.
...
// which is what stops such pods scheduling onto general pools. Registering one
// as well-known inverts that to "allowed when undefined" and dissolves the
// isolation with no error anywhere.
